...
$(document).ready(function(){
    $("#but1").click(function(){
        $.ajax({url: "php/index.php",data: {country: $("#countrys").val()}, success: function(result){
            $("#result").html("Capital: " + result["data"][0]['capital'] + "<br>" + "Population: " + result["data"][0]["population"]);
   }});
   });
    $("#but2").click(function(){
        $.ajax({url: "php/index2.php",data: {lat: $('#inputLat').val(), lng: $('#inputLon').val()}, success: function(result){
            $("#result").html("Sunrise: " + result["sunrise"] + "<br>" + "Sunset: " + result["sunset"]);
   }});
   });
    $("#but3").click(function(){
        $.ajax({url: "php/index3.php",data: {q: $("#inputCity").val()}, success: function(result){
